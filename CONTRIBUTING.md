...
`master` carries this file and the fork's README, and no mocha source. It is not a release and not a
copy of one: with two lines, letting either live on master would be arbitrary, and a copy on each
branch drifts. This file exists once, here, and the support branches point at it.

The release sources are the `support/*` branches, each release tagged `v<version>` on its own branch.
`prepublishOnly` on both lines runs `scripts/require-support-branch.js`, which refuses to publish from
anywhere else, so nothing is decided by master's state.

Agents: documentation changes go here. Code and releases go on a `support/*` branch — check
`git rev-parse --abbrev-ref HEAD` starts with `support/` before editing or publishing either.
